  ...
  useEffect(() =>{
    ...
    if (targetEls.length > 0 && triggerEls.length > 0) {
        ...
        Array.from(triggerEls).forEach((triggerEl, index) => {
          if (targetEls[index]) {  // Ensure there's a corresponding target element
    ...
    if (targetEls2.length > 0 && triggerEls2.length > 0) {
        ...
        Array.from(triggerEls2).forEach((triggerEl2, index) => {
          if (targetEls2[index]) {  // Ensure there's a corresponding target element
      ...
      <nav className="">
        <div className="flex flex-wrap items-center justify-between max-w-screen-xl mx-auto ">
          ...
          <div className="ms-auto pe-3">
            ...
            <button  className="inline-flex items-end p-2 w-10 h-10 justify-center text-sm text-white rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200" type="button" data-drawer-target="drawer-navigation" data-drawer-show="drawer-navigation" aria-controls="drawer-navigation">
                                      ...
                                      {child.childItems.nodes.map((grandchild)=>{
                                        return (
                                          <li key={grandchild.id}>
                                            <a href={routeMenu(grandchild)}  target={(grandchild.target ? grandchild.target : '' )}  className="flex items-center  text-black px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white ">
                                              {grandchild.label}
                                            </a>
                                          </li>
                                        );
