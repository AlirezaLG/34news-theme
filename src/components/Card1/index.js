...
const Card1 = ({ post, widget , style, size="medium"}) => {
  ...
  return (
    <div className=''>
        <div className="w-full bg-white rounded-md  ">
            ...
            <Link href={getNavigationUrl(post)}>
                <Image unoptimized width={img?.width} height={img?.height} style={`width:'100%', height:'auto'`}  className="rounded-t-lg" src={img?.url}  />
            </Link>
            <div className="py-3 px-2">
                <a href={getNavigationUrl(post)}>
                    <h4 className={`mb-2 ${style?.title_size ? "leading-7 text-"+style?.title_size : "text-lg leading-6" } font-bold tracking-tight text-gray-900  `}>{decode(post?.title?.rendered)}</h4>
                </a>
